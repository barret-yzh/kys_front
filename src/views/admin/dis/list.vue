<template>
	<div id="body" class="bodyClass">
			...
			<div style="padding:10px;">
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">评论内容</span>
					<div>
						<input type="input" placeholder="请输入评论内容" name="content" v-model="content"
		...
		<div class="tablebox">
				...
				<tbody id="disList">
					<template v-for="item in dataList">
						<tr>
							...
							<td v-html="'' + (item.poidStr == null ? '' : item.poidStr) + ''"></td>
							<td v-html="'' + (item.dis.content == null ? '' : item.dis.content) + ''"></td>
							<td v-html="'' + (item.dis.createTime == null ? '' : item.dis.createTime) + ''"></td>
							<td style="width:200px">
								<a href="javascript:void(0)" :data-id="item.dis.id" @click="del"
			...
			<div>
				<div id="dis_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/dis/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/dis/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"content": content,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/dis/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
