		...
		<div class="tabbody">
			<div class="bodyli active">
				<div class="inputbox" id="titleForm">
					...
					<div class="inputright">
						<input type="input" placeholder="请输入标题" name="title" v-model="title"
							class="form-control edit-input" id="title">
					</div>
				</div>
				...
				<div class="inputbox" id="zlIntroForm">
					...
					<div class="inputright">
						<textarea class="form-control edit-textarea" name="zlIntro" v-model="zlIntro"
							id="zlIntro"></textarea>
					</div>
				</div>
				<div class="inputbox" id="zlFileForm">
					...
					<div class="inputright">
						<div id="zlFile" class="dropzone" data-id="zlFile" max-num="1" file-type="4" max-size="1000"
							init-val="" :request-url="REQUEST_URL" :init-val="zlFile"></div>
					</div>
				</div>
			</div>
		</div>
...
export default {
	...
	mounted() {
		...
		setSelectableVal('zlType', that.zlType)
		setZoneImg2(zlFileDrop, that.zlFile);
	},
	methods: {
		getDataList() {
			var that = this;
		...
		queryDataDetail() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + "/admin/data_msg/queryDataDetail",
				async: false,
				data: {
					'id': that.id,
					'loginToken': that.currentLoginToken,
				...
				success: function (result) {
					...
					that.zlType = result.zlType;
					that.zlIntro = result.zlIntro;
					that.zlFile = result.zlFile;
					$("#zlIntro").val(result.zlIntro);
				}
			});
		},
		submitData() {
			...
			$.ajax({
				type: 'post',
				url: this.REQUEST_URL + "/admin/data_msg/update_submit",
				async: false,
				data: {
					'id': that.id,
					'title': title,
