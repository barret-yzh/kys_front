...
<template>
	<div id="body"  class="bodyClass">
		...
		<div>
			...
			<div style="padding:10px;">
				<a href="javascript:void(0)" :data-params="''" data-href="/admin/banner/add_page" @click="goToPage" style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-success" >新增</a>
			</div>
			<div style="padding:10px;">
			</div>
		</div>
			...
			<table class="table table-bordered">
				...
				<tbody  id="bannerList">
					<template v-for="item in dataList">
						<tr>
							<td>
								<template v-for="item2 in item.bannerList">
									<img :src="item2" style="width:80px;height:80px;display:inline;margin-left:4px;" @click="handleExportJsMethod"  data-methodtype="maxImg">
									</template>
								</td>
								<td style="width:200px">
									<a href="javascript:void(0)" :data-params="'id='+item.banner.id+''" data-href="/admin/banner/update_page" @click="goToPage" style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-primary" >修改</a>
									<a href="javascript:void(0)" :data-id="item.banner.id" @click="del" style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-danger" >删除</a>
								</td>
							</tr>
						</template>
						<template v-if="dataList==null || dataList.length==0">
							<div style="height:60px;"><h2 style="position:fixed;margin-left:47%;width:100%;border:1px">无数据</h2></div>
						</template>
					</tbody>
				</table>
				<div>
	...
	methods:{
		...
		del(e){
			...
			if(r==true){
				$.ajax({
					type : 'get',
					url : this.REQUEST_URL+"/admin/banner/del",
					async: false,
					data : {
		...
		ajaxCount(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/banner/queryCount',
				async: false,
				data : {
					"loginToken":that.currentLoginToken,
				},
		...
		ajaxList(page){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+'/admin/banner/queryList',
				async: false,
				data : {
					"page":page,
					"loginToken":that.currentLoginToken,
