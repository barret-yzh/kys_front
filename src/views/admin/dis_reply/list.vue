					...
					<template v-for="item in dataList">
						<tr>
							...
							<td style="width:200px">
								<a href="javascript:void(0)" :data-id="item.disReply.id" @click="del"
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/dis_reply/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/dis_reply/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"content": content,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/dis_reply/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
