<template>
	<div id="body" class="bodyClass">
		...
		<div>
			<div style="padding:10px;">
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">类型名</span>
					<div>
						<input type="input" placeholder="请输入类型名" name="typeName" v-model="typeName"
							class="form-control  query-input" id="typeName">
					</div>
					</p>
				</div>
				<div style="margin-left:20px;display:inline-block">
			...
			<div style="padding:10px;">
				<a href="javascript:void(0)" :data-params="''" data-href="/admin/zl_type/add_page" @click="goToPage"
		...
		<div class="tablebox">
			<table class="table table-bordered">
				...
				<tbody id="zlTypeList">
					<template v-for="item in dataList">
						<tr>
							<td v-html="'' + (item.zlType.typeName == null ? '' : item.zlType.typeName) + ''"></td>
							<td style="width:200px">
								<a href="javascript:void(0)" :data-params="'id=' + item.zlType.id + ''"
									data-href="/admin/zl_type/update_page" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-primary">修改</a>
								<a href="javascript:void(0)" :data-id="item.zlType.id" @click="del"
			...
			<div>
				<div id="zl_type_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/zl_type/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/zl_type/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"typeName": typeName,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/zl_type/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
