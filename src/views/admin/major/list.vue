<template>
	<div id="body" class="bodyClass">
		...
		<div>
				...
				<div style="margin-left:20px;display:inline-block">
					...
					<div>
						<input type="input" placeholder="请输入专业名" name="majorName" v-model="majorName"
							class="form-control  query-input" id="majorName">
					</div>
					</p>
				</div>
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">院校</span>
					<div>
						<select class="form-control query-select" data-id="schoolId" id="schoolId">
			...
			<div style="padding:10px;">
				<a href="javascript:void(0)" :data-params="''" data-href="/admin/major/add_page" @click="goToPage"
		...
		<div class="tablebox">
			<table class="table table-bordered">
				...
				<tbody id="majorList">
					<template v-for="item in dataList">
						<tr>
							<td v-html="'' + (item.major.majorName == null ? '' : item.major.majorName) + ''"></td>
							<td v-html="'' + (item.schoolIdStr == null ? '' : item.schoolIdStr) + ''"></td>
							<td v-html="'' + (item.major.majorIntro == null ? '' : item.major.majorIntro) + ''"></td>
							<td v-html="'' + (item.major.csProject == null ? '' : item.major.csProject) + ''"></td>
							<td>
								<template v-for="item2 in item.fsfaList">
									<a :href="item2" target="_blank">下载</a>
								</template>
							</td>
							<td v-html="'' + (item.major.dsMsg == null ? '' : item.major.dsMsg) + ''"></td>
							<td style="width:200px">
								<a href="javascript:void(0)" :data-params="'id=' + item.major.id + ''"
									data-href="/admin/major/update_page" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-primary">修改</a>
								<a href="javascript:void(0)" :data-id="item.major.id" @click="del"
			...
			<div>
				<div id="major_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		getInitData() {
			...
			$.ajax({
				...
				success: function (result) {
					that.schoolList = admin_utils.addAllSelect(result.schoolList);
				}
			});
		},
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/major/del",
					async: false,
					data: {
						"loginToken": that.currentLoginToken,
						"id": dataId
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/major/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"majorName": majorName,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/major/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
