<template>
	<div id="body" class="bodyClass">
		...
		<div>
			<div style="padding:10px;">
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">院校名</span>
					<div>
						<input type="input" placeholder="请输入院校名" name="name" v-model="name"
							class="form-control  query-input" id="name">
					</div>
					</p>
				</div>
				<div style="margin-left:20px;display:inline-block">
			...
			<div style="padding:10px;">
				<a href="javascript:void(0)" :data-params="''" data-href="/admin/school/add_page" @click="goToPage"
		...
		<div class="tablebox">
			<table class="table table-bordered">
				...
				<tbody id="schoolList">
					<template v-for="item in dataList">
						<tr>
							<td v-html="'' + (item.school.name == null ? '' : item.school.name) + ''"></td>
							<td>
								<template v-for="item2 in item.yxImgList">
							...
							<td v-html="'' + (item.school.yxIntro == null ? '' : item.school.yxIntro) + ''"></td>
							<td style="width:200px">
								<a href="javascript:void(0)" :data-params="'id=' + item.school.id + ''"
									data-href="/admin/school/update_page" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-primary">修改</a>
								<a href="javascript:void(0)" :data-id="item.school.id" @click="del"
			...
			<div>
				<div id="school_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/school/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/school/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"name": name,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/school/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
