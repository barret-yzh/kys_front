		...
		<div class="tablebox"  style="width:90%">
			<table class="table table-bordered">
				<tbody >
					<tr>
						<td class="col1">标题</td>
						<td class="col2" v-html="newsName==null?'':newsName"></td>
					</tr>
					<tr>
						<td class="col1">资讯附图</td>
						<td class="col2">
							<template v-for="item2 in newsImgList">
								<img :src="item2" style="width:200px;height:200px;"/>
							</template>
						</td>
					</tr>
					<tr>
						<td class="col1">资讯详情</td>
						<td class="col2" v-html="newsDetail==null?'':newsDetail"></td>
					</tr>
					<tr>
...
export default {
	...
	methods:{
		queryDataDetail(){
			...
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/news/getDetailData",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.newsName = (result.detail.news.newsName==null?'':result.detail.news.newsName);
					that.newsImgList = result.detail.newsImgList;
					that.newsDetail = (result.detail.news.newsDetail==null?'':result.detail.news.newsDetail);
					that.createTime = (result.detail.news.createTime==null?'':result.detail.news.createTime);
				}
			});
		},
	}
