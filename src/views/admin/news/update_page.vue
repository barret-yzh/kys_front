...
export default {
	...
	mounted() {
		...
		setZoneImg(newsImgDrop,that.newsImg);
		setEditorHtml('newsDetail',that.newsDetail)
	},
	methods:{
		queryDataDetail(){
			var that = this;
			$.ajax({
				type : 'get',
				url : this.REQUEST_URL+"/admin/news/queryDataDetail",
				async: false,
				data : {
					'id':that.id,
				...
				success : function(result) {
					that.newsName=result.newsName;
					that.newsImg=result.newsImg;
					that.newsDetail=result.newsDetail;
					setEditorHtml('newsDetail',result.newsDetail)
				}
			});
		},
		submitData(){
			...
			$.ajax({
				type : 'post',
				url : this.REQUEST_URL+"/admin/news/update_submit",
				async: false,
				data : {
					'id':that.id,
					'newsName':newsName,
