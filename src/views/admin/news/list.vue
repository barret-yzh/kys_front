<template>
	<div id="body" class="bodyClass">
		...
		<div>
			<div style="padding:10px;">
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">标题</span>
					<div>
						<input type="input" placeholder="请输入标题" name="newsName" v-model="newsName"
							class="form-control  query-input" id="newsName">
					</div>
					</p>
				</div>
				<div style="margin-left:20px;display:inline-block">
			...
			<div style="padding:10px;">
				<a href="javascript:void(0)" :data-params="''" data-href="/admin/news/add_page" @click="goToPage"
		...
		<div class="tablebox">
			<table class="table table-bordered">
				...
				<tbody id="newsList">
					<template v-for="item in dataList">
						<tr>
							<td v-html="'' + (item.news.newsName == null ? '' : item.news.newsName) + ''"></td>
							<td>
								<template v-for="item2 in item.newsImgList">
							...
							<td v-html="'' + (item.news.createTime == null ? '' : item.news.createTime) + ''"></td>
							<td style="width:200px">
								<a href="javascript:void(0)" :data-params="'id=' + item.news.id + ''"
									data-href="/admin/news/detail" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-success">查看详情</a>
								<a href="javascript:void(0)" :data-params="'id=' + item.news.id + ''"
									data-href="/admin/news/update_page" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-primary">修改</a>
								<a href="javascript:void(0)" :data-id="item.news.id" @click="del"
			...
			<div>
				<div id="news_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/news/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/news/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"newsName": newsName,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/news/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
