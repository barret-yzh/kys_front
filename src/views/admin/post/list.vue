						...
						<tr>
							...
							<td style="width:200px">
								<a href="javascript:void(0)" :data-params="'id=' + item.post.id + ''"
									data-href="/admin/post/detail" @click="goToPage"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-success">查看详情</a>
								<a href="javascript:void(0)" :data-id="item.post.id" @click="del"
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/admin/post/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/post/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"title": title,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/admin/post/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
