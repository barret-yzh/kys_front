	...
	<div class="bodyClass">
		<article>
			...
			<div class="search" style="border: 1px solid #ccc!important;display:flex">
				...
				<input @click="queryPostList(1)" class="input_submit" value="搜索" type="button">
			</div>
			<div class="clear blank"></div>
			<!-- tuijian-news begin-->
						...
						<div class="tuijian-news-list" style="width:100%">
							<ul  id="dataQl"  >
								<template v-for="(item,index) in ql">
									<li>
										<a  data-href="/index/post_detail" :data-params="'id='+item.model.id+''" @click="goToPage" style="height:80px;">
...
export default {
	...
	methods:{
		queryPostList(page){
			...
			$.ajax({
				type : 'get',
				url : that.REQUEST_URL+"/index/post_search/queryPostList",
				async: false,
				data : {
					'title':title,
					"page":page,
