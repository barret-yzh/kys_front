	...
	<div id="body" class="bodyClass">
			...
			<div style="padding:10px;">
				<div style="margin-left:20px;display:inline-block">
					<p style="margin-top:30px;">
						<span style="">收藏时间</span>
					<div style="display:flex">
						<input type="date" id="createTime1" class="form-control  query-input">
						&nbsp;&nbsp;~&nbsp;&nbsp;<input type="date" id="createTime2" class="form-control  query-input">
					</div>
					</p>
				</div>
				<div style="margin-left:20px;display:inline-block">
		...
		<div class="tablebox">
			<table class="table table-bordered">
				...
				<tbody id="collectLogList">
					<template v-for="item in dataList">
						<tr>
							<td v-html="'' + (item.pidStr == null ? '' : item.pidStr) + ''"></td>
							<!-- <td v-html="'' + (item.uidStr == null ? '' : item.uidStr) + ''"></td> -->
							<td
								v-html="'' + (item.collectLog.createTime == null ? '' : item.collectLog.createTime) + ''">
							</td>
							<td style="width:200px">
								<a href="javascript:void(0)"
									:data-targetpage="'/index/post_detail?id=' + item.collectLog.pid + ''"
									data-href="/index" @click="goToChildPageWithBlank"
									style="margin-left:5px;margin-top:3px;" class="btnStyle1 btn btn-success">查看详情</a>
								<a href="javascript:void(0)" :data-id="item.collectLog.id" @click="del"
			...
			<div>
				<div id="collect_log_bar" class="pagination" style="margin-top:20px;"></div>
			</div>
		</div>
	</div>
</template>
	...
	methods: {
		...
		del(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/user/collect_log/del",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/user/collect_log/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/user/collect_log/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
