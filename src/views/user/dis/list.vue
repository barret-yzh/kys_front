					...
					<template v-for="item in dataList">
						<tr>
							...
							<td style="width:200px">
								<a href="javascript:void(0)" :data-id="item.dis.id" @click="del1"
	...
	methods: {
		...
		del1(e) {
			...
			if (r == true) {
				$.ajax({
					type: 'get',
					url: this.REQUEST_URL + "/user/dis/del1",
					async: false,
					data: {
		...
		ajaxCount() {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/user/dis/queryCount',
				async: false,
				data: {
					"loginToken": that.currentLoginToken,
					"content": content,
		...
		ajaxList(page) {
			...
			$.ajax({
				type: 'get',
				url: this.REQUEST_URL + '/user/dis/queryList',
				async: false,
				data: {
					"page": page,
					"loginToken": that.currentLoginToken,
